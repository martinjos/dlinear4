...
TEST_F(SymbolicExpressionTest, GetTermsInAddition) {
  ...
  const map<Expression, mpq_class> terms{get_expr_to_coeff_map_in_addition(e)};
...
TEST_F(SymbolicExpressionTest, StaticConstant) {
  EXPECT_MPQ_EQ_DOUBLE(Expression::Zero().Evaluate(), 0.0);
  EXPECT_MPQ_EQ_DOUBLE(Expression::One().Evaluate(), 1.0);
  EXPECT_MPQ_NEAR(Expression::Pi().Evaluate(), M_PI, 0.000001);
  EXPECT_MPQ_NEAR(Expression::E().Evaluate(), M_E, 0.000000001);
}
...
TEST_F(SymbolicExpressionTest, UnaryMinus) {
  ...
  EXPECT_EQ(c3_.Evaluate(), -(-c3_).Evaluate());
  EXPECT_PRED2(ExprEqual, c3_, -(-c3_));
  EXPECT_EQ(c3_.Evaluate(), (-(-c3_)).Evaluate());
  ...
  EXPECT_EQ(e.Evaluate(env), (-(-e)).Evaluate(env));
...
TEST_F(SymbolicExpressionTest, Inc3) {
  ...
  EXPECT_MPQ_EQ_DOUBLE((c1++).Evaluate(), 3.1415);
  EXPECT_MPQ_EQ_DOUBLE(c1.Evaluate(), 3.1415 + 1.0);

  Expression c2{3.1415};
  EXPECT_MPQ_EQ_DOUBLE((++c2).Evaluate(), 3.1415 + 1.0);
  EXPECT_MPQ_EQ_DOUBLE(c2.Evaluate(), 3.1415 + 1.0);
}
...
TEST_F(SymbolicExpressionTest, Log) {
  EXPECT_MPQ_EQ_DOUBLE(log(pi_).Evaluate(), std::log(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(log(one_).Evaluate(), std::log(1.0));
  EXPECT_MPQ_EQ_DOUBLE(log(zero_).Evaluate(), std::log(0.0));
...
TEST_F(SymbolicExpressionTest, Abs) {
  EXPECT_MPQ_EQ_DOUBLE(abs(pi_).Evaluate(), std::fabs(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(abs(one_).Evaluate(), std::fabs(1.0));
  EXPECT_MPQ_EQ_DOUBLE(abs(zero_).Evaluate(), std::fabs(0.0));
  EXPECT_MPQ_EQ_DOUBLE(abs(neg_one_).Evaluate(), std::fabs(-1.0));
  EXPECT_MPQ_EQ_DOUBLE(abs(neg_pi_).Evaluate(), std::fabs(-M_PI));
  const Expression e{abs(x_ * y_ * pi_) + abs(x_) + abs(y_)};
  const Environment env{{var_x_, -2}, {var_y_, 3.2}};
  EXPECT_MPQ_EQ_DOUBLE(e.Evaluate(env), std::fabs(-2 * 3.2 * M_PI) +
...
TEST_F(SymbolicExpressionTest, Exp) {
  EXPECT_MPQ_EQ_DOUBLE(exp(pi_).Evaluate(), std::exp(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(exp(one_).Evaluate(), std::exp(1));
  EXPECT_MPQ_EQ_DOUBLE(exp(zero_).Evaluate(), std::exp(0));
  EXPECT_MPQ_EQ_DOUBLE(exp(neg_one_).Evaluate(), std::exp(-1));
  EXPECT_MPQ_EQ_DOUBLE(exp(neg_pi_).Evaluate(), std::exp(-M_PI));
...
TEST_F(SymbolicExpressionTest, Sqrt2) {
  EXPECT_MPQ_EQ_DOUBLE(sqrt(pi_).Evaluate(), std::sqrt(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(sqrt(one_).Evaluate(), std::sqrt(1.0));
  EXPECT_MPQ_EQ_DOUBLE(sqrt(zero_).Evaluate(), std::sqrt(0.0));
...
TEST_F(SymbolicExpressionTest, Pow1) {
  ...
  // (x^y)^z => (x^y)^z
  EXPECT_PRED2(ExprNotEqual, pow(pow(x_, y_), z_), pow(x_, y_ * z_));
}

TEST_F(SymbolicExpressionTest, Pow2) {
  EXPECT_MPQ_EQ_DOUBLE(pow(pi_, pi_).Evaluate(), std::pow(M_PI, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(pow(pi_, one_).Evaluate(), std::pow(M_PI, 1));
  EXPECT_MPQ_EQ_DOUBLE(pow(pi_, two_).Evaluate(), std::pow(M_PI, 2));
  EXPECT_MPQ_EQ_DOUBLE(pow(pi_, zero_).Evaluate(), std::pow(M_PI, 0));
  EXPECT_MPQ_EQ_DOUBLE(pow(pi_, neg_one_).Evaluate(), std::pow(M_PI, -1));
  EXPECT_MPQ_EQ_DOUBLE(pow(pi_, neg_pi_).Evaluate(), std::pow(M_PI, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(pow(one_, pi_).Evaluate(), std::pow(1, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(pow(one_, one_).Evaluate(), std::pow(1, 1));
  EXPECT_MPQ_EQ_DOUBLE(pow(one_, two_).Evaluate(), std::pow(1, 2));
  EXPECT_MPQ_EQ_DOUBLE(pow(one_, zero_).Evaluate(), std::pow(1, 0));
  EXPECT_MPQ_EQ_DOUBLE(pow(one_, neg_one_).Evaluate(), std::pow(1, -1));
  EXPECT_MPQ_EQ_DOUBLE(pow(one_, neg_pi_).Evaluate(), std::pow(1, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(pow(two_, pi_).Evaluate(), std::pow(2, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(pow(two_, one_).Evaluate(), std::pow(2, 1));
  EXPECT_MPQ_EQ_DOUBLE(pow(two_, two_).Evaluate(), std::pow(2, 2));
  EXPECT_MPQ_EQ_DOUBLE(pow(two_, zero_).Evaluate(), std::pow(2, 0));
  EXPECT_MPQ_EQ_DOUBLE(pow(two_, neg_one_).Evaluate(), std::pow(2, -1));
  EXPECT_MPQ_EQ_DOUBLE(pow(two_, neg_pi_).Evaluate(), std::pow(2, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(pow(zero_, pi_).Evaluate(), std::pow(0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(pow(zero_, one_).Evaluate(), std::pow(0, 1));
  EXPECT_MPQ_EQ_DOUBLE(pow(zero_, two_).Evaluate(), std::pow(0, 2));
  EXPECT_MPQ_EQ_DOUBLE(pow(zero_, zero_).Evaluate(), std::pow(0, 0));
  EXPECT_MPQ_EQ_DOUBLE(pow(zero_, neg_one_).Evaluate(), std::pow(0, -1));
  EXPECT_MPQ_EQ_DOUBLE(pow(zero_, neg_pi_).Evaluate(), std::pow(0, -M_PI));

  EXPECT_THROW(pow(neg_one_, pi_).Evaluate(), domain_error);
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_one_, one_).Evaluate(), std::pow(-1, 1));
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_one_, two_).Evaluate(), std::pow(-1, 2));
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_one_, zero_).Evaluate(), std::pow(-1, 0));
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_one_, neg_one_).Evaluate(), std::pow(-1, -1));
  EXPECT_THROW(pow(neg_one_, neg_pi_).Evaluate(), domain_error);

  EXPECT_THROW(pow(neg_pi_, pi_).Evaluate(), domain_error);
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_pi_, one_).Evaluate(), std::pow(-M_PI, 1));
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_pi_, two_).Evaluate(), std::pow(-M_PI, 2));
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_pi_, zero_).Evaluate(), std::pow(-M_PI, 0));
  EXPECT_MPQ_EQ_DOUBLE(pow(neg_pi_, neg_one_).Evaluate(), std::pow(-M_PI, -1));
  ...
  const Environment env{{var_x_, 2}, {var_y_, 3.2}};
  EXPECT_MPQ_EQ_DOUBLE(e1.Evaluate(env), std::pow(2 * 3.2 * M_PI, 2 + 3.2 + M_PI));
  EXPECT_MPQ_EQ_DOUBLE(e2.Evaluate(env),
                   std::pow(2, 2) * std::pow(3.2, 2) * std::pow(2, 3.2));
}

TEST_F(SymbolicExpressionTest, Sin) {
  EXPECT_MPQ_EQ_DOUBLE(sin(pi_).Evaluate(), std::sin(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(sin(one_).Evaluate(), std::sin(1));
  EXPECT_MPQ_EQ_DOUBLE(sin(two_).Evaluate(), std::sin(2));
  EXPECT_MPQ_EQ_DOUBLE(sin(zero_).Evaluate(), std::sin(0));
  EXPECT_MPQ_EQ_DOUBLE(sin(neg_one_).Evaluate(), std::sin(-1));
  EXPECT_MPQ_EQ_DOUBLE(sin(neg_pi_).Evaluate(), std::sin(-M_PI));
...
TEST_F(SymbolicExpressionTest, Cos) {
  EXPECT_MPQ_EQ_DOUBLE(cos(pi_).Evaluate(), std::cos(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(cos(one_).Evaluate(), std::cos(1));
  EXPECT_MPQ_EQ_DOUBLE(cos(two_).Evaluate(), std::cos(2));
  EXPECT_MPQ_EQ_DOUBLE(cos(zero_).Evaluate(), std::cos(0));
  EXPECT_MPQ_EQ_DOUBLE(cos(neg_one_).Evaluate(), std::cos(-1));
  EXPECT_MPQ_EQ_DOUBLE(cos(neg_pi_).Evaluate(), std::cos(-M_PI));
...
TEST_F(SymbolicExpressionTest, Tan) {
  EXPECT_MPQ_EQ_DOUBLE(tan(pi_).Evaluate(), std::tan(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(tan(one_).Evaluate(), std::tan(1));
  EXPECT_MPQ_EQ_DOUBLE(tan(two_).Evaluate(), std::tan(2));
  EXPECT_MPQ_EQ_DOUBLE(tan(zero_).Evaluate(), std::tan(0));
  EXPECT_MPQ_EQ_DOUBLE(tan(neg_one_).Evaluate(), std::tan(-1));
  EXPECT_MPQ_EQ_DOUBLE(tan(neg_pi_).Evaluate(), std::tan(-M_PI));
...
TEST_F(SymbolicExpressionTest, Asin) {
  ...
  EXPECT_MPQ_EQ_DOUBLE(asin(one_).Evaluate(), std::asin(1));
  EXPECT_THROW(asin(two_).Evaluate(), domain_error);
  EXPECT_MPQ_EQ_DOUBLE(asin(zero_).Evaluate(), std::asin(0));
  EXPECT_MPQ_EQ_DOUBLE(asin(neg_one_).Evaluate(), std::asin(-1));
  ...
  EXPECT_MPQ_EQ_DOUBLE(e.Evaluate(env), std::asin(0.2 * 0.3 * M_PI) +
...
TEST_F(SymbolicExpressionTest, Acos) {
  ...
  EXPECT_MPQ_EQ_DOUBLE(acos(one_).Evaluate(), std::acos(1));
  EXPECT_THROW(acos(two_).Evaluate(), domain_error);
  EXPECT_MPQ_EQ_DOUBLE(acos(zero_).Evaluate(), std::acos(0));
  EXPECT_MPQ_EQ_DOUBLE(acos(neg_one_).Evaluate(), std::acos(-1));
  EXPECT_THROW(acos(neg_pi_).Evaluate(), domain_error);

  const Expression e{acos(x_ * y_ * pi_) + acos(x_) + acos(y_)};
  const Environment env{{var_x_, 0.2}, {var_y_, 0.3}};
  EXPECT_MPQ_EQ_DOUBLE(e.Evaluate(env), std::acos(0.2 * 0.3 * M_PI) +
                                        std::acos(0.2) + std::acos(0.3));
  EXPECT_EQ((acos(x_)).to_string(), "acos(x)");
}

TEST_F(SymbolicExpressionTest, Atan) {
  EXPECT_MPQ_EQ_DOUBLE(atan(pi_).Evaluate(), std::atan(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(atan(one_).Evaluate(), std::atan(1));
  EXPECT_MPQ_EQ_DOUBLE(atan(two_).Evaluate(), std::atan(2));
  EXPECT_MPQ_EQ_DOUBLE(atan(zero_).Evaluate(), std::atan(0));
  EXPECT_MPQ_EQ_DOUBLE(atan(neg_one_).Evaluate(), std::atan(-1));
  EXPECT_MPQ_EQ_DOUBLE(atan(neg_pi_).Evaluate(), std::atan(-M_PI));

  const Expression e{atan(x_ * y_ * pi_) + atan(x_) + atan(y_)};
  const Environment env{{var_x_, 0.2}, {var_y_, 0.3}};
  EXPECT_MPQ_EQ_DOUBLE(e.Evaluate(env), std::atan(0.2 * 0.3 * M_PI) +
                                        std::atan(0.2) + std::atan(0.3));
  EXPECT_EQ((atan(x_)).to_string(), "atan(x)");
}

TEST_F(SymbolicExpressionTest, Atan2) {
  EXPECT_MPQ_EQ_DOUBLE(atan2(pi_, pi_).Evaluate(), std::atan2(M_PI, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(atan2(pi_, one_).Evaluate(), std::atan2(M_PI, 1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(pi_, two_).Evaluate(), std::atan2(M_PI, 2));
  EXPECT_MPQ_EQ_DOUBLE(atan2(pi_, zero_).Evaluate(), std::atan2(M_PI, 0));
  EXPECT_MPQ_EQ_DOUBLE(atan2(pi_, neg_one_).Evaluate(), std::atan2(M_PI, -1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(pi_, neg_pi_).Evaluate(), std::atan2(M_PI, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(atan2(one_, pi_).Evaluate(), std::atan2(1, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(atan2(one_, one_).Evaluate(), std::atan2(1, 1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(one_, two_).Evaluate(), std::atan2(1, 2));
  EXPECT_MPQ_EQ_DOUBLE(atan2(one_, zero_).Evaluate(), std::atan2(1, 0));
  EXPECT_MPQ_EQ_DOUBLE(atan2(one_, neg_one_).Evaluate(), std::atan2(1, -1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(one_, neg_pi_).Evaluate(), std::atan2(1, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(atan2(two_, pi_).Evaluate(), std::atan2(2, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(atan2(two_, one_).Evaluate(), std::atan2(2, 1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(two_, two_).Evaluate(), std::atan2(2, 2));
  EXPECT_MPQ_EQ_DOUBLE(atan2(two_, zero_).Evaluate(), std::atan2(2, 0));
  EXPECT_MPQ_EQ_DOUBLE(atan2(two_, neg_one_).Evaluate(), std::atan2(2, -1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(two_, neg_pi_).Evaluate(), std::atan2(2, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(atan2(zero_, pi_).Evaluate(), std::atan2(0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(atan2(zero_, one_).Evaluate(), std::atan2(0, 1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(zero_, two_).Evaluate(), std::atan2(0, 2));
  EXPECT_MPQ_EQ_DOUBLE(atan2(zero_, zero_).Evaluate(), std::atan2(0, 0));
  EXPECT_MPQ_EQ_DOUBLE(atan2(zero_, neg_one_).Evaluate(), std::atan2(0, -1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(zero_, neg_pi_).Evaluate(), std::atan2(0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_one_, pi_).Evaluate(), std::atan2(-1, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_one_, one_).Evaluate(), std::atan2(-1, 1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_one_, two_).Evaluate(), std::atan2(-1, 2));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_one_, zero_).Evaluate(), std::atan2(-1, 0));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_one_, neg_one_).Evaluate(), std::atan2(-1, -1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_one_, neg_pi_).Evaluate(), std::atan2(-1, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_pi_, pi_).Evaluate(), std::atan2(-M_PI, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_pi_, one_).Evaluate(), std::atan2(-M_PI, 1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_pi_, two_).Evaluate(), std::atan2(-M_PI, 2));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_pi_, zero_).Evaluate(), std::atan2(-M_PI, 0));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_pi_, neg_one_).Evaluate(), std::atan2(-M_PI, -1));
  EXPECT_MPQ_EQ_DOUBLE(atan2(neg_pi_, neg_pi_).Evaluate(),
                   std::atan2(-M_PI, -M_PI));
...
TEST_F(SymbolicExpressionTest, Sinh) {
  EXPECT_MPQ_EQ_DOUBLE(sinh(pi_).Evaluate(), std::sinh(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(sinh(one_).Evaluate(), std::sinh(1));
  EXPECT_MPQ_EQ_DOUBLE(sinh(two_).Evaluate(), std::sinh(2));
  EXPECT_MPQ_EQ_DOUBLE(sinh(zero_).Evaluate(), std::sinh(0));
  EXPECT_MPQ_EQ_DOUBLE(sinh(neg_one_).Evaluate(), std::sinh(-1));
  EXPECT_MPQ_EQ_DOUBLE(sinh(neg_pi_).Evaluate(), std::sinh(-M_PI));
...
TEST_F(SymbolicExpressionTest, Cosh) {
  EXPECT_MPQ_EQ_DOUBLE(cosh(pi_).Evaluate(), std::cosh(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(cosh(one_).Evaluate(), std::cosh(1));
  EXPECT_MPQ_EQ_DOUBLE(cosh(two_).Evaluate(), std::cosh(2));
  EXPECT_MPQ_EQ_DOUBLE(cosh(zero_).Evaluate(), std::cosh(0));
  EXPECT_MPQ_EQ_DOUBLE(cosh(neg_one_).Evaluate(), std::cosh(-1));
  EXPECT_MPQ_EQ_DOUBLE(cosh(neg_pi_).Evaluate(), std::cosh(-M_PI));
...
TEST_F(SymbolicExpressionTest, Tanh) {
  EXPECT_MPQ_EQ_DOUBLE(tanh(pi_).Evaluate(), std::tanh(M_PI));
  EXPECT_MPQ_EQ_DOUBLE(tanh(one_).Evaluate(), std::tanh(1));
  EXPECT_MPQ_EQ_DOUBLE(tanh(two_).Evaluate(), std::tanh(2));
  EXPECT_MPQ_EQ_DOUBLE(tanh(zero_).Evaluate(), std::tanh(0));
  EXPECT_MPQ_EQ_DOUBLE(tanh(neg_one_).Evaluate(), std::tanh(-1));
  EXPECT_MPQ_EQ_DOUBLE(tanh(neg_pi_).Evaluate(), std::tanh(-M_PI));
...
TEST_F(SymbolicExpressionTest, Min2) {
  EXPECT_MPQ_EQ_DOUBLE(min(pi_, pi_).Evaluate(), std::min(M_PI, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(min(pi_, one_).Evaluate(), std::min(M_PI, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(pi_, two_).Evaluate(), std::min(M_PI, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(min(pi_, zero_).Evaluate(), std::min(M_PI, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(min(pi_, neg_one_).Evaluate(), std::min(M_PI, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(pi_, neg_pi_).Evaluate(), std::min(M_PI, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(min(one_, pi_).Evaluate(), std::min(1.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(min(one_, one_).Evaluate(), std::min(1.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(one_, two_).Evaluate(), std::min(1.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(min(one_, zero_).Evaluate(), std::min(1.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(min(one_, neg_one_).Evaluate(), std::min(1.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(one_, neg_pi_).Evaluate(), std::min(1.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(min(two_, pi_).Evaluate(), std::min(2.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(min(two_, one_).Evaluate(), std::min(2.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(two_, two_).Evaluate(), std::min(2.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(min(two_, zero_).Evaluate(), std::min(2.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(min(two_, neg_one_).Evaluate(), std::min(2.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(two_, neg_pi_).Evaluate(), std::min(2.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(min(zero_, pi_).Evaluate(), std::min(0.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(min(zero_, one_).Evaluate(), std::min(0.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(zero_, two_).Evaluate(), std::min(0.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(min(zero_, zero_).Evaluate(), std::min(0.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(min(zero_, neg_one_).Evaluate(), std::min(0.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(zero_, neg_pi_).Evaluate(), std::min(0.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(min(neg_one_, pi_).Evaluate(), std::min(-1.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_one_, one_).Evaluate(), std::min(-1.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_one_, two_).Evaluate(), std::min(-1.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_one_, zero_).Evaluate(), std::min(-1.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_one_, neg_one_).Evaluate(), std::min(-1.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_one_, neg_pi_).Evaluate(), std::min(-1.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(min(neg_pi_, pi_).Evaluate(), std::min(-M_PI, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_pi_, one_).Evaluate(), std::min(-M_PI, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_pi_, two_).Evaluate(), std::min(-M_PI, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(min(neg_pi_, zero_).Evaluate(), std::min(-M_PI, 0.0));
...
TEST_F(SymbolicExpressionTest, Max2) {
  EXPECT_MPQ_EQ_DOUBLE(max(pi_, pi_).Evaluate(), std::max(M_PI, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(max(pi_, one_).Evaluate(), std::max(M_PI, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(pi_, two_).Evaluate(), std::max(M_PI, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(max(pi_, zero_).Evaluate(), std::max(M_PI, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(max(pi_, neg_one_).Evaluate(), std::max(M_PI, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(pi_, neg_pi_).Evaluate(), std::max(M_PI, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(max(one_, pi_).Evaluate(), std::max(1.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(max(one_, one_).Evaluate(), std::max(1.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(one_, two_).Evaluate(), std::max(1.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(max(one_, zero_).Evaluate(), std::max(1.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(max(one_, neg_one_).Evaluate(), std::max(1.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(one_, neg_pi_).Evaluate(), std::max(1.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(max(two_, pi_).Evaluate(), std::max(2.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(max(two_, one_).Evaluate(), std::max(2.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(two_, two_).Evaluate(), std::max(2.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(max(two_, zero_).Evaluate(), std::max(2.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(max(two_, neg_one_).Evaluate(), std::max(2.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(two_, neg_pi_).Evaluate(), std::max(2.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(max(zero_, pi_).Evaluate(), std::max(0.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(max(zero_, one_).Evaluate(), std::max(0.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(zero_, two_).Evaluate(), std::max(0.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(max(zero_, zero_).Evaluate(), std::max(0.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(max(zero_, neg_one_).Evaluate(), std::max(0.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(zero_, neg_pi_).Evaluate(), std::max(0.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(max(neg_one_, pi_).Evaluate(), std::max(-1.0, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_one_, one_).Evaluate(), std::max(-1.0, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_one_, two_).Evaluate(), std::max(-1.0, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_one_, zero_).Evaluate(), std::max(-1.0, 0.0));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_one_, neg_one_).Evaluate(), std::max(-1.0, -1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_one_, neg_pi_).Evaluate(), std::max(-1.0, -M_PI));

  EXPECT_MPQ_EQ_DOUBLE(max(neg_pi_, pi_).Evaluate(), std::max(-M_PI, M_PI));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_pi_, one_).Evaluate(), std::max(-M_PI, 1.0));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_pi_, two_).Evaluate(), std::max(-M_PI, 2.0));
  EXPECT_MPQ_EQ_DOUBLE(max(neg_pi_, zero_).Evaluate(), std::max(-M_PI, 0.0));
